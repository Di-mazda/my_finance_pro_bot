...
(function () {
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    // НОВОЕ: если initData пустая - страница открыта не через нажатие
    // кнопки в Telegram (например, ссылку скопировали и вставили в
    // обычный браузер). В этом случае сервер всё равно ответит 401, но
    // сообщать об этом сразу и понятно - лучше, чем заставлять человека
    // читать код ошибки и логи Railway.
    if (!tg || !tg.initData) {
      showFatalError(
